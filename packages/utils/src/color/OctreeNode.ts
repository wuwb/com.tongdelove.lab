...
export class OctreeNode {
  ...
  private _reduce(level: number = 0) {
    if (this.isLeaf && this.level > level) {
      if (this.parent) {
        // 信息传递到上一级
        this.parent?.acc.push([
          this.count,
          this.level === this.maxDepth ? this.value : this.accValue,
        ])
      ...
      this.children.forEach((child, i, children) => {
        ...
        if (children.length - 1 === i && child.dead) {
          ...
          this.accValue = pad(
            ((parseInt(this.value, 2) << leftOffset) + this.evalAcc).toString(
              2
            ),
            (this.maxDepth - this.level + 1) * 3
          )
          this.isLeaf = true
        }
      })
  ...
  private binColors(_ = { value: '', count: 0 }): any {
    if (this.isLiveLeaf) {
      return {
        value: _.value + this.accValue,
        count: this.count + _.count,
      }
    } else if (this.isLiveNode) {
      return this.children.map((child) =>
        child.binColors({
          value: _.value + this.value,
          count: this.count + _.count,
        })
      )
    }
  ...
  get themeColors(): Array<[number, number, number]> {
    const binColors = flattenDeep(this.binColors())
      .sort((a: any, b: any) => b.count - a.count) //从多到少
      .map((item: any) => item.value)
    // console.log(binColors.slice(0, 5))
    return binColors.map((binColor: any[]) => {
      let r = ''
      let g = ''
      let b = ''
  ...
  private get evalAcc(): number {
    return Math.round(
      this.acc.reduce((acc, countValue) => {
        const count = countValue?.[0]
        const value = countValue?.[1]
        return acc + parseInt(value ?? '', 2) * (count ?? 0)
      }, 0) / this.count
    )
  }
}
