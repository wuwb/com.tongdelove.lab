...
diff_match_patch.prototype.diff_compute_ = function (c, b, q, p) {
  ...
  if (f != -1) {
    j = [
      [DIFF_INSERT, m.substring(0, f)],
      [DIFF_EQUAL, d],
      [DIFF_INSERT, m.substring(f + d.length)],
    ];
...
diff_match_patch.prototype.diff_fromDelta = function (b, j) {
  ...
  if (a != b.length) {
    throw new Error(
      'Delta length (' + a + ') does not equal source text length (' + b.length + ').',
    );
  }
  return f;
};
...
diff_match_patch.prototype.patch_addContext_ = function (f, e) {
  ...
  var d = 0;
  while (
    e.indexOf(b) != e.lastIndexOf(b) &&
    b.length < this.Match_MaxBits - this.Patch_Margin - this.Patch_Margin
  ) {
    d += this.Patch_Margin;
  ...
  for (var n = 0; n < a.length; n++) {
    ...
    if (e.length > this.Match_MaxBits) {
      ...
      if (f != -1) {
        k = this.match_main(
          p,
          e.substring(e.length - this.Match_MaxBits),
          b + e.length - this.Match_MaxBits,
        );
        if (k == -1 || f >= k) {
          f = -1;
        }
    ...
    if (f == -1) {
      ...
      if (e == d) {
        ...
        if (
          e.length > this.Match_MaxBits &&
          this.diff_levenshtein(i) / e.length > this.Patch_DeleteThreshold
        ) {
          g[n] = false;
        } else {
