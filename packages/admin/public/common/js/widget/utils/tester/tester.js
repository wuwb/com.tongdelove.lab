...
function run(runingData) {
  ...
  describe(testFile, function () {
    ...
    if (!runingData['fn']['methods']) {
      ...
      typeof runingData['fn']['methods'] === 'object' &&
      runingData['fn']['methods'].length > 0
    ) {
      // object function
          ...
          describe('#' + subMethod + '()', function () {
            ...
            it(testFile + ' ' + subMethod + ' check', function () {
              for (var j = 0, l = testData.length; j < l; j++) {
                if (typeof testData[j]['return'] === 'object') {
                  testMod[subMethod]
                    .apply(testMod, testData[j]['params'])
                    .should.deep.equal(testData[j]['return']);
                } else {
                  testMod[subMethod]
                    .apply(testMod, testData[j]['params'])
                    .should.equal(testData[j]['return']);
                }
              }
            });
          });
