...
function init() {
  ...
  /** * 添加相机 * @type {THREE.PerspectiveCamera} */
  camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.1, 1000);
  /** * 添加一个渲染器 * @type {THREE.CSS3DRenderer} */
  ...
  var sides = [
    { url: './images/posx.jpg', position: [-512, 0, 0], rotation: [0, Math.PI / 2, 0] },
  ...
  for (var i = 0; i < sides.length; i++) {
    ...
    // 2 pixels extra to close the gap. imgElement.height = 1028;
  ...
  /** * 注册监听 */ document.addEventListener('mousedown', onDocumentMouseDown, false);
...
/** * 实时渲染函数 */ function render() {
  requestAnimationFrame(render);
  lon = Math.max(-180, Math.min(180, lon));
  //限制固定角度内旋转 lon += 0.1;
  //自动旋转 lat = Math.max(-85, Math.min(85, lat));
...
/** * 窗体大小改变 */ function onWindowResize() {
...
/** * 鼠标滚轮改变相机焦距 */ function onDocumentMouseWheel(event) {
...
/** * 脚本入口 * @type {init} */ window.onload = init;
