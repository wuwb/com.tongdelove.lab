...
class Comments extends React.Component {
  ...
  async topicLike() {
    let response = await API.topicLike({
      ...
      status: this.props.topicLike ? 0 : 1,
    });
  ...
  async _handleKeyPress(event) {
    if (event.key === 'Enter') {
      ...
      this.props.addComments({
        replyContent: this.state.replyContent,
        replyName: this.state.userInfo.username,
        index: this.props.topicIndex,
      });
  ...
  render() {
        ...
        <ul className={`comments-list ${this.props.dialog && 'fill'}`}>
          ...
          {this.props.discuss.map((item, index) => {
            ...
            if (this.props.dialog || index !== 3) {
              return (
                <li
                  className={`content ${index > 3 && !this.props.dialog && 'hidden'} ${
                    this.state.showMoreComments && 'no-hidden'
                  }`}
                  key={index}
                >
                  ...
                  {this.props.discuss.length > 4 ? (
                    <li
                      className={`content show-more u-f-lightblack2 ${
                        this.state.showMoreComments && 'hidden'
                      }`}
                    >
    ...
    return (
      <div className={Style['comments-section']}>
        {this.props.dialog ? <CommentsList /> : ''}
        <div className="opetions">
          <div className="fl-left">
            <span
              className={`favorite  ${this.props.topicLike && 'active'}`}
              onClick={this.topicLike.bind(this)}
            ></span>
