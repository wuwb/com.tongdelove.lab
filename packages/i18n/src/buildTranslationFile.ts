...
export async function buildTranslationFile(config: BuildConfig) {
  ...
  mainstream.on('finish', () => {
    let transFile = fs.readFileSync(
      resolve(config.output, defaultTranslationPath), {
      encoding: 'utf8',
    })
    if (config.autoResolveConflict === true) {
      transFile = resolveCodeConflict(transFile)
    }
    const transJson = JSON.parse(transFile)
...
export function resolveCodeConflict(data: string) {
