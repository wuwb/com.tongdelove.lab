...
export class JwtService extends BaseJwtService {
     ...
     * @return 是否有效
     */
    static consume(token: string, issuer: string, appSecret: string, audience: string, allowedClockSkewSeconds: number) {
     ...
     * @throws Exception
     */
    static produce(issuer: string, appSecret: string, audience: string, expirationTimeMinutesInTheFuture: number = 10) {
        if (isBlank(issuer) || isBlank(appSecret) || isBlank(audience)) {
