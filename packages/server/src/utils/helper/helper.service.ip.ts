...
import { COMMON_SERVICE } from '@/config/admin.config';
...
export class IPService {
  ...
  private queryIPByAliyun(ip: IP): Promise<IIPDetail> {
    return this.httpService.axiosRef
      .request({
        headers: {
          Authorization: `APPCODE ${COMMON_SERVICE.aliyunIPAuth}`,
        },
  ...
  private queryIPByJUHE(ip: IP): Promise<any> {
    return this.httpService.axiosRef
      .get<any>(`http://apis.juhe.cn/ip/ipNew?ip=${ip}&key=${COMMON_SERVICE.juheIPAuth}`)
      .then((response) => {
