...
import {
    ...
    Irefunds2,
    ICertificates,
} from './interface/pay.interface';
import { IcombineH5, IcombineNative, IcombineApp, IcombineJsapi, IcloseSubOrders } from './interface/combine.interface';
import { BatchesTransfer, ProfitSharing } from './interface/pay-v2.interface';
import { BasePayService } from './BasePayService';
import { objectToQueryString } from '@/utils/string.util';

export class PaySerivce extends BasePayService {
    ...
    public async query_batches_transfer_list_wx(
        ...
        const url = baseUrl + objectToQueryString(params, ['batch_id']);
        const authorization = this.init('GET', url);
    ...
    public async query_batches_transfer_detail_wx(
        ...
        const url = baseUrl + objectToQueryString(params, ['batch_id', 'detail_id']);
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    /**
    ...
    public async query_batches_transfer_list(
        ...
        const url = baseUrl + objectToQueryString(params, ['out_batch_no']);
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    /**
    ...
    public async query_batches_transfer_detail(
        ...
        const url = baseUrl + objectToQueryString(params, ['out_batch_no', 'out_detail_no']);
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    //#endregion 商家转账到零钱
    ...
    public async query_profitsharing_orders(transaction_id: string, out_order_no: string): Promise<ProfitSharing.CreateProfitSharingOrders.IOutput> {
        ...
        url = url + objectToQueryString({ transaction_id });
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    /**
    ...
    public async query_profitsharing_return_orders(
        ...
        url = url + objectToQueryString({ out_order_no });
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    /**
    ...
    public async profitsharing_bills(bill_date: string, tar_type?: string): Promise<ProfitSharing.ProfitSharingBills.IOutput> {
        ...
        url = url + objectToQueryString({ bill_date, ...(tar_type && { tar_type }) });
        const authorization = this.init('GET', url);
        return await this.getRequestV2(url, authorization);
    }
    //#endregion 分账
