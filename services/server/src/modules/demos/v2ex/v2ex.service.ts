...
export class V2exService {
    ...
    async getTopicDetail1(params: any) {
        try {
            ...
            if (Number(params.p) === 1) {
                ...
                tag_link = $(box).first().find('.header .chevron').next().attr('href') || '';
                tag_link = tag_link.split('/')[2];
        ...
        try {
            ...
            if (res) {
                ...
                if (!is_sign_in) {
                    ...
                    const data: any = await $v2exHttp.get(`/mission/daily/redeem?${once}`, {
                        headers: {
    ...
    async getUserNotifications(cookie: string) {
        try {
            ...
            const $ = load(res.data);
            const $money = $('#money');
            const notifications = $money?.prev()?.text()?.split('条未读提醒')[0]?.trim();
