...
export class A5Service {
    ...
    async spider() {
        const response = await this.httpService.axiosRef.get('https://www.a5.cn/tasklist.html', {
            headers: {
        ...
        $('.m-tk-list ul li').each((index, item) => {
            ...
            console.log('date: ', new Prisma.Decimal(+$item.find('.col-sm-7 h3 a i.fa-cny').text().replace('&nbsp;', '')));
