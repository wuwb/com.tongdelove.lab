...
export class FreelancerController {
  ...
  async getSubscribe(@Body() body: CreateSubscribeDto, @User() user) {
    ...
    if (
      ...
      body.wechatWebhook.webhook.indexOf(
  ...
  async subscribeTest(@Body() body: CreateSubscribeDto) {
    ...
    // 获取 tasks 表第一条数据进行推搡
    if (body.dingdingWebhook.webhook !== '') {
      await this.dingdingService.sendTextToWebhook(
        `${body.dingdingWebhook.webhook}?access_token=${body.dingdingWebhook.secret}`,
        '测试内容'
      )
    }
    ...
    if (body.feishuWebhook.webhook !== '') {
      await this.dingdingService.sendTextToWebhook(
        `${body.feishuWebhook.webhook}?access_token=${body.feishuWebhook.secret}`,
        '订阅测试'
      )
    }

    if (body.wechatWebhook.webhook !== '') {
      let result = await this.webhookService.sendText(
        body.wechatWebhook.webhook,
        '订阅测试'
      )
