...
describe('RolesGuard', () => {
    ...
    it('should return true if the `HasRoles` decorator is set', async () => {
        jest
            .spyOn(reflector, 'get')
            .mockImplementation((a: any, b: any) => [RoleEnum.User]);
        const context = createMock<ExecutionContext>({
            getHandler: jest.fn(),
    ...
    it('should return false if the `HasRoles` decorator is set but role is not allowed', async () => {
        jest.spyOn(reflector, 'get').mockReturnValue([RoleEnum.Admin]);
        const request = {
            user: { roles: [RoleEnum.User] },
        };
        const context = createMock<ExecutionContext>();
...
describe('RolesGuard(ts-mockito)', () => {
    ...
    it('should skip(return true) if the `HasRoles` decorator is not set', async () => {
        ...
        when(
            reflecter.get<RoleEnum[]>(HAS_ROLES_KEY, contextInstacne.getHandler()),
        ).thenReturn([] as RoleEnum[]);
    ...
    it('should return true if the `HasRoles` decorator is set', async () => {
        ...
        when(
            reflecter.get<RoleEnum[]>(HAS_ROLES_KEY, contextInstacne.getHandler()),
        ).thenReturn([RoleEnum.User] as RoleEnum[]);
    ...
    it('should return false if the `HasRoles` decorator is set but role is not allowed', async () => {
        ...
        when(
            reflecter.get<RoleEnum[]>(HAS_ROLES_KEY, contextInstacne.getHandler()),
        ).thenReturn([RoleEnum.Admin] as RoleEnum[]);
