...
export class AccountService {
     ...
     * 根据账号 id 修改密码
     */
    async modifyPassWordById(id: string, modifyPasswordDto: ModifyPasswordDto): Promise<string> {
        // if (id === 1) {
        ...
        const findResult = await getConnection()
            ...
            .getRawOne();
        if (findResult?.password && this.helperService.checkPassword(password, findResult?.password)) {
            const {
                raw: { affectedRows },
            } = await this.accountRepository.update(id, {
     ...
     * 根据账号 id 修改账号信息
     */
    async modifyById(id: string, updateAccountDto: UpdateAccountDto): Promise<string> {
        // if (id === 1) {
     ...
     */
    async recycleOrBanUser(id: string, action: 'recycle' | 'ban'): Promise<string> {
        // if (id === 1) {
        //     throw new HttpException('系统默认生成的账号不能删除', HttpStatus.OK);
        // }
        let status = 1;
     ...
     */
    async revertRecycleOrBanUser(id: string): Promise<string> {
        // if (id === '1') {
     ...
     * 根据账号 id 查询账号信息
     */
    async accountById(id: string): Promise<AccountResDto | null> {
