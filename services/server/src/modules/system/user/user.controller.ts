...
export class UserController {
  ...
  async findUsers(
    ...
    const data = await this.userService.findUsers(query, page, limit, args)
  ...
  // 用户关注了那些人
  @Get('/:id/follows')
  async userFollows(@CurrentUser() user, @Param('id') id: string) {
    //
  }

  // 用户有哪些粉丝
  @Get('/:id/followers')
  async userFollowers(@CurrentUser() user, @Param('userId') userId: string) {
    // 判断用户是否不存在，
    // 不存在报错
  }
  ...
  @RateLimit({
    points: 3,
    duration: 300,
    errorMessage: '你的速度太快了，请等待五分钟后再重试。',
  })
  @Get('verify-updated-email')
  async verifyUpdatedEmail(@Query('token') token: string) {
    return this.userService.verifyUpdatedEmail(token)
  }
}
