...
export class TaobaoOrderRawController {
  ...
  async uploadTaobaoOrderCSV(@UploadedFile() file: Express.Multer.File) {
    ...
    try {
      ...
      // TODO 将原始文件保存到 oss
      result = {
        url: 'http://printlake.oss-cn-beijing.aliyuncs.com/egg-multipart-test/ExportOrderList202101261213.csv',
        fields: {},
      }
    } catch (err) {
    ...
    let parsedRecords = records.map((item) => {
      return {
        ...
        telephone: item[17],
        cellphone: item[18],
        virtualNumberExpirationAt:
          !!item[19] && item[19] !== null && item[19] !== 'null'
            ? new Date(item[19])
            : new Date(0),
        orderCreatedAt:
          !!item[20] && item[20] !== null && item[20] !== 'null'
            ? new Date(item[20])
            : new Date(0),
        payAt:
          item[21] && item[21] !== null && item[21] !== 'null'
            ? new Date(item[21])
            : new Date(0),
        goodsTitle: item[22],
        ...
        confirmReceiptAt:
          !!item[54] && item[54] !== null && item[54] !== 'null'
            ? new Date(item[54])
            : new Date(0),
        remitNum:
          !!item[55] && item[55] !== null && item[55] !== 'null'
            ? Number(`${item[55]}`.replace('元', ''))
            : 0,
        ...
        deliveryAt:
          !!item[68] && item[68] !== null && item[68] !== 'null'
            ? new Date(item[68])
            : new Date(0),
  ...
  async parseClientInfo() {
    ...
    for (const item of data) {
      let isExist = await this.customerService.exist(
        item.clientName,
        item.clientTaobao
      )
      if (!isExist) {
