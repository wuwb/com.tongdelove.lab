...
Wall.prototype.init = function() {
  ...
  if ((!this.settings.emptyBrick && appendLen <= 0)
    || appendLen % this.settings.offset !== 0) {
    this.loaded = true;
    return;
  }
  this.listen();
...
Wall.prototype.listen = function() {
  var self = this,
    ...
    globalHeight, scrollY, clientHeight;
  ...
  var load = function() {
    ...
    if (!isDomScroll) {
      ...
      globalHeight = (self.$wrap.offset().top - self.settings.$croller.offset().top) + self.$wrap.height();
      scrollY = self.$wrap.scrollTop();
      clientHeight = self.settings.$croller.height();
    }

    if (!self.loaded && !self.locker
  ...
  self.settings.$croller
    .off('scroll.wall' + self.settings.stamp)
    .on('scroll.wall' + self.settings.stamp, _.throttle(function() {
      load();
    }, 100));
...
Wall.prototype.render = function() {
  ...
  .done(function(json) {
    ...
    util.verify(json, function(result) {
      ...
      if (appendLen <= 0
        || appendLen % self.settings.offset !== 0) {
        self.stop();
        return;
      }
    });
