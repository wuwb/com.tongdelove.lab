...
module.exports = {
    getKey: function (seed) {
        var k_hash = CryptoJS.MD5(seed) + '';
        var k_sub = k_hash.substr(0, 16);
        return  CryptoJS.enc.Utf8.parse(k_sub);
   ...
   * @returns {string}
   */
    encryptLocal:function(seed,text){
       seed=CryptoJS.MD5(seed.toUpperCase()) + '';
       var data={
         '_data_':text,
         '_time_':new Date().getTime()+600*1000
       };
       data=JSON.stringify(data);
       return this.encrypt(seed,data);
    },
    decryptLocal:function(seed,text) {
      seed = CryptoJS.MD5(seed.toUpperCase()) + '';
      var data = JSON.parse(this.decrypt(seed, text)),
     ...
     * @returns {string}
     */
    encrypt: function (seed, text) {
        var key, iv;
        key = iv = this.getKey(seed);

        return encryptString(key, iv, text);
    },
    /**
     * 加密以字符串为value的对象
     * @param {string} seed
     ...
     * @returns {Object}
     */
    encryptData: function encryptData(seed, data) {
        ...
        for (var dataKey in data) {
            if (data.hasOwnProperty(dataKey) && data[dataKey] !== null) {
                data[dataKey] = encryptString(key, iv, data[dataKey].toString());
            }
        }
     ...
     * @returns {string}
     */
    decrypt: function (seed, text) {
        var key, iv;
        key = iv = this.getKey(seed);

        return decryptString(key, iv, text.replace(/\s+/g, ""));
    },
    /**
     * 解密以字符串为value的对象
     * @param {string} seed
     ...
     * @returns {Object}
     */
    decryptData: function decryptData(seed, data) {
        var key, iv;
        key = iv = this.getKey(seed);

        for (var dataKey in data) {
            if (data.hasOwnProperty(dataKey) && data[dataKey] !== null) {
                data[dataKey] = decryptString(key, iv, data[dataKey].toString());
            }
        }
     ...
     */
    getSrandNum: function getSrandNum(callback, customApi) {
        $.ajax({
            url: customApi ? customApi : api['getSrandNum'],
            type: "POST",
            dataType: 'json',
            success: function (seed) {
                if (seed.status.code === 1001) {
                    typeof callback === 'function' && callback(seed.result);
                }
                else {
                    tips.show(seed.result || seed.status.msg);
                }
            },
            error: function () {
                tips.show('网络错误，请重试');
