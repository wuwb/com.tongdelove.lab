...
module.exports = {
  ...
  verify: function(data, cb, failcb, fn_notice, context) {
    if (data && data.status) {
      if (data.status.code === 1001) {
        ...
        var msg = data.result || data.status.msg || '未知错误，可能的原因：1.网络问题 2.服务器超时 3.其他原因';
  ...
  formatTime: function(dateObj, format) {
    ...
    return formatTime
      .replace(/YYYY/img, year)
      .replace(/mm/img, addZero(month))
      .replace(/dd/img, addZero(date))
      .replace(/HH/img, addZero(hour))
      .replace(/ii/img, addZero(minute))
      .replace(/ss/img, addZero(second))
      .replace(/YY/img, ('' + year).substr(2))
      .replace(/m/img, month)
      .replace(/d/img, date)
      .replace(/H/img, hour)
