...
const LocaleSwitch = ({ pageContext }) => {
  ...
  return (
    <div ref={select} className="relative ml-4">
      <button
        type="button"
        className="hover:bg-primary-50 hover:text-primary-600 focus:bg-primary-50 focus:text-primary-600 flex h-full w-20 cursor-pointer items-center justify-between rounded-md px-2 py-2 focus:outline-none"
        onClick={() => setShowing(!showing)}
      >
        <WorldIcon />
        <span className="capitalize">{locale}</span>
        <RiArrowDownSLine className="text-primary-600 ml-1" />
      </button>
      <div
            ...
            return (
              <Link href={href} key={locale} locale={locale} passHref>
                <p
                  ...
                  className="hover:bg-primary-50 hover:text-primary-600 hover:text-primary-600 cursor-pointer rounded-md p-2 text-center capitalize"
                >
                  {locale}
                </p>
              </Link>
