...
(function (factory) {
  ...
  editormd.prototype = editormd.fn = {
    ...
    init: function (id, options) {
      ...
      settings.pluginPath =
        settings.pluginPath === '' ? settings.path + '../plugins/' : settings.pluginPath;
      ...
      var appendElements = [
        !settings.readOnly
          ? '<a href="javascript:;" class="fa fa-close ' + classPrefix + 'preview-close-btn"></a>'
          : '',
    ...
    save: function () {
      ...
      if (settings.watch || (!settings.watch && state.preview)) {
        ...
        if (settings.toc) {
          var tocContainer =
            settings.tocContainer === '' ? previewContainer : $(settings.tocContainer);
  ...
  editormd.toolbarHandlers = {
    ...
    datetime: function () {
      ...
      var datefmt =
        ...
        ' ' +
        editormd.dateFormat(
          langName === 'zh-cn' || langName === 'zh-tw' ? 'cn-week-day' : 'week-day',
        );
  ...
  editormd.markedRenderer = function (markdownToC, options) {
      ...
      if (atLinkReg.test(text)) {
        ...
        if (settings.emailLink) {
          text = text.replace(emailLinkReg, function ($1, $2, $3, $4, $5) {
            return !$2 && $.inArray($5, 'jpg|jpeg|png|gif|webp|ico|icon|pdf'.split('|')) < 0
              ? '<a href="mailto:' + $1 + '">' + $1 + '</a>'
              : $1;
          });
        }
    ...
    markedRenderer.heading = function (text, level, raw) {
      ...
      var headingHTML =
        '<h' + level + ' id="h' + level + '-' + this.options.headerPrefix + id + '">';
    ...
    markedRenderer.paragraph = function (text) {
      ...
      if (!isTeXLine && isTeXInline) {
        text = text.replace(/(\$\$([^\$]*)\$\$)+/g, function ($1, $2) {
          return (
            '<span class="' + editormd.classNames.tex + '">' + $2.replace(/\$/g, '') + '</span>'
          );
        });
      } else {
  ...
  editormd.createDialog = function (options) {
    ...
    if (typeof options.buttons === 'object') {
      ...
      for (var key in options.buttons) {
        ...
        footer.append(
          '<button class="' + classPrefix + 'btn ' + btnClassName + '">' + btn[0] + '</button>',
        );
