...
export async function roleList(options?: { [key: string]: any }) {
  return request<API.RoleAccountListDto[]>('/api/asystem/account/account-role', {
    method: 'GET',
    ...(options || {}),
  });
}
...
export async function accountRoleListByAccountId(
  ...
  return request<API.AccountRoleListResDto[]>(`/api/asystem/account/account-role/${param0}`, {
    method: 'GET',
    params: { ...queryParams },
    ...(options || {}),
  });
