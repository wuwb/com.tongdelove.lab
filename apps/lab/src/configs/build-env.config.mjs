...
import { z } from 'zod'
import { getValidatedBuildEnv, truthyStrEnvValue, zConvertTruthyStrToBool } from '../lib/env/index.mjs'
...
const schema = {
  NEXT_BUILD_ENV_OUTPUT: z
    ...
    .default('classic'),
  NEXT_BUILD_ENV_TSCONFIG: z.string().endsWith('.json').default('tsconfig.json'),
  ...
  NEXT_BUILD_ENV_BUILD_ID: z.string().default(isProd ? new Date().toISOString().replace(':', '_') : ''),
  // --------------------------------------------------------------------
  // Sentry related
  // --------------------------------------------------------------------
