  ...
  return (
    <div>
      <div className="flex flex-col divide-y divide-neutral-100 pb-24 pr-24">
        {appleGuidedata && (
          <div className="pb-12">
            {appleGuidedata?.map((category, index) => selectedCategory === index && <Category key={category.name} data={category} />)}
          </div>
        )}
      </div>

      <Indexes data={appleGuidedata} selectedCategory={selectedCategory} target={target} setTarget={setTarget} setEnableScrollIntoView={setEnableScrollIntoView} />
    </div>
  )
}
