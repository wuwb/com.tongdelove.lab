...
export const LoginForm: FC<LoginFormProps> = ({ redirectToPage }) => {
  ...
  const onSubmit = async (formValues: LoginFormValues): Promise<void> => {
    ...
    const { ok = false, status = 500, error = 'Server or network Error' } = result ?? {}
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-backgroundSecondary mx-auto flex w-full max-w-lg flex-col rounded-xl border border-border p-4 sm:p-20">
      ...
      <div className="form-group">
        ...
        <div className="form-field">
          <div className="form-control justify-between">
            ...
            <label className="form-label">
              <a className="link link-primary link-underline-hover text-sm">Forgot your password?</a>
            </label>
          </div>
        </div>
        <div className="form-field pt-5">
          <div className="form-control justify-between">
            <button type="submit" disabled={isSubmitting} className={clsx('btn btn-primary w-full', isSubmitting ? '' : 'bg-indigo-600')}>
              Sign in
            </button>
            {error && <div>{error}</div>}
          </div>
        ...
        <div className="form-field">
          <div className="form-control">
            <a className="link link-primary link-underline-hover text-sm">Don't have an account? Sign in</a>
          </div>
        </div>
      </div>
    </form>
