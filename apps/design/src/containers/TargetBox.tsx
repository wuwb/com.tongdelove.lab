...
const TargetBox = memo((props) => {
  ...
  const { scaleNum, rotateNum, canvasId, dispatch, dragState, setDragState } =
    props

  const { designData, currentDesignData, flip, lineGuides } = state
          ...
          >
            <div className={styles.layerWrap}>
              ...
              <div>
                {designData.map((item) => {
                  return (
                    <Elements key={item.uuid} item={item} scaleNum={scaleNum} />
                  )
                })}
              </div>

              <div className={styles.bgAndElementWrap}></div>
