...
export const Usage = () => {
  ...
  const generateMockData = (days: number) => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      data.push({ date: date.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' }), requests: Math.floor(Math.random() * 1000) + 100, cost: Math.random() * 50 + 10, tokens: Math.floor(Math.random() * 50000) + 5000 })
    }
    return data
  }
      ...
      <div style={{
        ...
        <TabsPrimitive.Root defaultValue="logs" className="w-full">
          <TabsPrimitive.List className="flex border-b border-gray-200 mb-4">
            <TabsPrimitive.Trigger className="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 data-[state=active]:text-blue-600 data-[state=active]:border-b-2 data-[state=active]:border-blue-600">
              请求日志
